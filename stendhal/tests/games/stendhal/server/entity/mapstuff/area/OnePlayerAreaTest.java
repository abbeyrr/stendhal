...
public class OnePlayerAreaTest {
	...
	/**
	 * Test exiting the area with a sheep
	 */
	@Test
	public void testOnExitWithSheep() {
		Player player = PlayerTestHelper.createPlayer("test");
		
		StendhalRPZone zone = new StendhalRPZone("test zone", 20, 20);
		MockStendlRPWorld.get().addRPZone(zone);
		
		StendhalRPAction.placeat(zone, player, 20, 20);
		
		final Sheep sheep = new Sheep(player);
		StendhalRPAction.placeat(zone, sheep, 1, 1); // when the sheep is far away
		assertFalse(player.isZoneChangeAllowed());
		
		StendhalRPAction.placeat(zone, sheep, 19, 19); // when the sheep is close
		assertTrue(player.isZoneChangeAllowed());
	}
