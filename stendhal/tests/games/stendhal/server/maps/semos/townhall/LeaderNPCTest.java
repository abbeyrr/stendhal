...
public class LeaderNPCTest {
	...
	public void testQuest() {
		...
		assertTrue(en.step(player, "weapon"));
		assertEquals("That club of thorns is a powerful weapon, it has a good damage to rate ratio. Despite the fast rate being useful, the low attack will not help you against strong creatures. Something heavier would be better then. The negative lifesteal of -0.1 will drain your health as you use it.", getReply(npc));
		player.drop(weapon4);
		...
		assertTrue(en.step(player, "weapon"));
		assertEquals("That black sword is a powerful weapon, it has a good damage to rate ratio. Despite the fast rate being useful, the low attack will not help you against strong creatures. Something heavier would be better then.", getReply(npc));
		player.drop(weapon6);
		...
		assertTrue(en.step(player, "weapon"));
		assertEquals("That obsidian knife is a powerful weapon, it has a good damage to rate ratio. It should be useful against strong creatures. Remember though that something weaker but faster may suffice against lower level creatures.", getReply(npc));
		player.drop(weapon7);
