...
public class FruitsForCoralia extends AbstractQuest {
	...
	private void prepareBringingStep() {
		...
		npc.add(ConversationStates.QUESTION_2,
			...
			public void fire(final Player player,
				...
				checkForEverything(player, npc, completeAction);
			}
		});
	...
	private void checkForEverything(final Player player, final EventRaiser npc, ChatAction completeAction) {
		final String specificQuest= player.getQuest(QUEST_SLOT);


		final ItemCollection array= new ItemCollection();


		array.addFromQuestStateString(specificQuest);


		for (final Map.Entry<String, Integer> ab : array.entrySet()) {
			new CollectRequestedItemsAction(ab.getKey(), QUEST_SLOT,
