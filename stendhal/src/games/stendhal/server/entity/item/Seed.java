...
public class Seed extends StackableItem {
	...
	public boolean onUsed(final RPEntity user) {
		// right now you can plant a seed on top of another one
		// we should check before planting the seed that the ground is empty
		
		Iterator<games.stendhal.server.entity.Entity> iterator = user.getZone().getEntitiesAt(this.getX(), this.getY()).iterator();
		boolean canPlantSeed = true;
		while (iterator.hasNext()) {
			games.stendhal.server.entity.Entity element = iterator.next();
			
			if (element.getName() == "growing_entity_spawner"){
				user.sendPrivateText(NotificationType.SERVER, "There already is a seed planted in here!");
				canPlantSeed = false;
			}
//			System.out.println("ItemClass: " + element.get(getItemClass()));
//			System.out.println("Name: " + element.getName());
//			System.out.println("Description: " + element.getDescription());
//          System.out.println("Item on ("+ user.getX() + " - " + user.getY() + "):" + iterator.next());
        }
