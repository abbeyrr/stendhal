...
public class OutfitChangerBehaviour extends MerchantBehaviour {
	 ...
	 */
	@Override
	public boolean transactAgreedDeal(ItemParserResult res, final EventRaiser seller, final Player player, int c) {
		...
		if(outfitType=="trunks"&& c>4) {
			seller.say("you choose a wrong number");
			return false;
		}
		if(outfitType=="swimsuit"&& c>4) {
			seller.say("you choose a wrong number");
			return false;
		}
		if(outfitType=="mask"&& c>6) {
			seller.say("you choose a wrong number");
			return false;
		}	
		if(outfitType=="gown"&& c>2) {
			seller.say("you choose a wrong number");
			return false;
		}		
		if(outfitType=="suit"&& c>1) {
			seller.say("you choose a wrong number");
			return false;
		}		
		...
		if (player.isEquipped("money", charge)) {
			...
			final Outfit newoutfit=possibleNewOutfits.get(c-1);
			
			
			putOnOutfit(player, newoutfit,outfitType );
			return true;
		} else {
	 ...
	 */
	public void putOnOutfit(final Player player, final Outfit newoutfit, final String outfitType) {
